...
const LocalTransferPage = () => {
    ...
    async function resolveAcc(accountNum){
        try{
        const response = await axios.get(`https://mentorship-payment-app.herokuapp.com/api/v1/transfers/resolveLocalAccount/${accountNum}`);
...
useEffect(()=>{
    ...
    if((accountNumber.length < 10 || accountNumber.length > 10) && accountName){
        setUser({
            ...user,
            accountName: ''
        });
...
const header = {'Authorization': localStorage.getItem("accessToken")}
    const saveTransaction = (e) => {
        ...
        LocalTransferService.saveTransaction(user, header).then((response) => {
            ...
            console.log(error)
            addToast('An error occured please try again later!!', { appearance: 'error' });
        })
    }
    ...
    <div className='localTransferPage'>
        ...
        <div className="localTransferPage__wrapper">
            ...
            <div className='localTransferPage__transfers' >
                <p className={`${activeTab ? 'localTransferPage__local' : "" }`} onClick={()=>setActiveTab(!activeTab)}>Local Transfer</p>
                <p className={`${activeTab ? '' : 'localTransferPage__local' }`} onClick={()=>setActiveTab(!activeTab)} >Other Bank Transfer</p>
            </div>
            <hr></hr>
            ...
            <div className="localTransferPage__body">
               ...
               {errorMessage && <div><h5>{errorMessage}</h5></div>}
                ...
                <div className="localTransferPage__content">
                    <button 
                        ...
                        className="localTransferPage__button" >Send Money</button>
                </div>
            </div>
